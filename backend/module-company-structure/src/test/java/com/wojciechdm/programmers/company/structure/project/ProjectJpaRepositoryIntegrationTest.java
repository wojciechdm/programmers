...
class ProjectJpaRepositoryIntegrationTest {
  ...
  private static ProjectJpaRepository projectJpaRepository;
  ...
  static void init() {
    ...
    projectJpaRepository = new ProjectJpaRepository(entityManagerFactory);
  }
  ...
  void shouldCrudProject() {
    ...
    // when
    Project actualSaved = projectJpaRepository.save(expectedSavedFirst);
    ...
    // when
    boolean actualExists = projectJpaRepository.exists(2L);
    // then
    ...
    // when
    long actualCount = projectJpaRepository.count();
    // then
    assertEquals(expectedCount, actualCount);

    // fetch
    // when
    Project actualFetch = projectJpaRepository.fetch(2L).get();
    ...
    Project expectedSavedSecond =
        new Project(
            ...
            LocalDate.now(),
            1L);
    projectJpaRepository.save(expectedSavedSecond);
    ...
    // when
    List<Project> actualFetchByClient = projectJpaRepository.fetchByClientId(1L);
    ...
    // fetch all
    // when
    List<Project> actualFetchAll =
        projectJpaRepository.fetchAll(2, 1, ProjectSortProperty.ID, false);
    ...
    // when
    Project actualUpdated = projectJpaRepository.update(expectedUpdated);
    ...
    // when
    boolean actualDeleted = projectJpaRepository.delete(2L);
    // then
